...
class Dbh {
	...
	public function itemedit($name, $color, $category, $subcategory, $description, $brand, $size, $pid){

		$sql = 'UPDATE product SET p_name=?, color=?, category=?, subcategory=?, description=?, brand=?, c_sizes=? WHERE p_id=?;';
		
		$stmt->$this->connect()->prepare($sql);
		$stmt->execute([$name, $color, $category, $subcategory, $description, $brand, $size, $pid]);
	}
